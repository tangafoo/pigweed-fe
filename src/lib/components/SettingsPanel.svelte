<script lang="ts">
	...
	import UserFlipCard from '$lib/components/UserFlipCard.svelte';
	import type { Session, SubscriptionSummary } from '@meteorclass/pigweed-contract';

	// The account/settings dashboard tab (/users/[id]?tab=settings). The Farm ID
	// card (now the flip card's settings face) sits up top; passkeys + sign-out
	// live below. `subscription` feeds the card's flip side.
	interface SettingsPanelProps {
		user: Session['user'];
		subscription?: SubscriptionSummary | null;
	}
	let { user, subscription = null }: SettingsPanelProps = $props();
	...
	// Tracks which passkey-id is currently in confirm-delete state, plus any
	// per-row delete error. Inline confirm beats a native dialog — the brand says
	// "no Apple-clean", so we don't reach for window.confirm.
...
<div class="mb-6 flex flex-col gap-6 lg:flex-row lg:items-start">
	<!-- Farm ID ⟷ subscription flip card. The extra padding is the "stage" the 3D
	     tilt plays in. -->
	<div class="shrink-0 px-2">
		<UserFlipCard {user} {subscription} front="settings" />
	</div>
